...
For approved independent preparation or an eligible multi-file build, use one coordinator and at most two focused workers; no nested managers. Keep help/status and ordinary single-image revisions with one agent. Prefer `gpt-5.6-sol` at medium effort for production workers and `gpt-5.6-luna` for narrow support, retaining the coordinator for direction and complete-stage review; use available capabilities and report a material fallback. New-stage generation follows the packet's content/reference and asset-contract prerequisites. From AF02 onward, asset production ends with five validated PNGs and the versioned asset-ready handoff. Keep source regions, anchors, facing, crops, animation and usable provisional metadata with asset production. Iterative character-relative grounding, flight height, hitbox fairness, Jump/Slide timing, difficulty and spawn balancing belong to editor-next calibration. Repeated zero-hit courses are not an artwork handoff gate. The user notifies the Workbench agent, which imports the handoff; the user then calibrates and saves. Asset readiness, artwork acceptance, calibration and playable-release acceptance are separate; pending checks cannot be reported as passed. Preserve strict later release validation and genuine renderer/scheduler regression tests. Pause independent legacy hazard tuning; preserve the eleven comparison candidates and their evidence with no silent promotion, overwrite or rollback. CI and Pages remain publication gates, not pre-generation gates. Coordinate schema changes with the actual Workbench importer.

Give workers fresh scoped context, pinned stage/file rules and actual reference pixels. They own isolated image outputs only; the coordinator owns shared registry/state, integration and publication. Follow the coordinator job procedure in the runbook. Do not claim measured parallel image throughput or a fixed usage budget before the Africa AF01–AF03 trial (AF01 is its first checkpoint).
