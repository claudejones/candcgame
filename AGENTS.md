...
For approved independent preparation or an eligible multi-file build, use one coordinator and at most two focused workers; no nested managers. Keep help/status and ordinary single-image revisions with one agent. Prefer `gpt-5.6-sol` at medium effort for production workers and `gpt-5.6-luna` for narrow support, retaining the coordinator for direction and complete-stage review; use available capabilities and report a material fallback. New-stage generation follows the packet's content/reference, contract-adoption and runtime/validation prerequisites. The user has delegated existing calibration review to a separate auto-calibration workstream and authorized Africa production to proceed; do not recreate that review as a generation blocker or overwrite its concurrent work. The intended calibration interface is editor-next. Pause independent legacy hazard tuning; preserve the eleven deployed comparison candidates and supporting tests until that workstream reviews them, with no silent promotion, overwrite or rollback. New-stage collision checks and deployed user approval remain required. CI and Pages remain publication gates after integration, not pre-generation gates.
