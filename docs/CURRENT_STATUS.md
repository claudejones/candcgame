...
All registered NA/SA/EU landscape production is accepted. The user approved shared Africa preparation and clarified that the trial covers **AF01–AF03**, with AF01 as the first checkpoint. The shared catalog now registers 21 IDs and activates only complete releases; new-stage source anchors, editor/save support and structural hazard validators are implemented and fixture-tested. No new artwork was generated, integrated or accepted.

Remaining production gates are concrete: (1) finish the existing dependent calibration review and formally promote the landscape contract; (2) retrieve/inspect the Africa reference pixels and attach them to the focused jobs. Africa selections/source research are recorded; direct reference-image downloads returned 403 or HTML here, so pixel readiness is not claimed. The agent completes reference work; the user supplies no paths or research fields. `status AF01` only reports this state. Once gates are met, the production command is `In claudejones/candcgame: build stage AF01.` The same setup then serves AF02/AF03 and later continents.

## Deferred and protected

Approved expansion direction: `docs/REMAINING_CONTINENTS_PLAN.md` and `config/remaining-continent-proposal.json` define four continents, 12 stages and five PNG files per stage (FAR/MID/GROUND, two ground hazards in one atlas, one flying atlas). They include the approved focal-point/life/uniqueness/scale rules and bounded execution policy. Africa duplicate/replacement choices are agent-selected and researched; the remaining nine briefs still need selection review. Africa actual visual reference packs remain pending retrieval/inspection. AF03 uses white stork and excludes pigeons. Production stays disabled until the explicit readiness fields are satisfied; plan approval does not reopen accepted landscapes or establish full Phase 8 completion.
