...
`In claudejones/candcgame: build landscape EU03.` EU02 — France / Paris is approved at reviewed revision `961d3911c756c9084114ab95255a4a06cfe489c0`. Its three image hashes, zero offsets and source anchors are unchanged. Deployed Design/Test/Game, scrolling through the finish and Inspector geometry passed. Evidence is in `docs/phase8-qa/EU02_LANDSCAPE_QA.md`; exact approval/hash pointers are in `config/asset-workflow-state.json`. EU03 — Spain / Barcelona remains pending. Europe regression follows EU03 acceptance.
