...
## 2026-09-19 — Remaining-stage focal points, life, uniqueness and scale

The user requires a recognizable local FAR focal point, a restrained scene-specific MID sign of life, distinct elements/hazards across the campaign and believable size at actual gameplay scale. Paris's focal landmark, a small settlement and the Amazon snake illustrate the principle; they are not motifs to repeat. Background wildlife is static/noninteractive scenery, not a baked gameplay hazard. Select references and assess visible subject size relative to characters, architecture and depth in the existing composite review; do not rescue an unsuitable subject by arbitrary scaling or altered geometry/physics.

AF03's proposed rock pigeon is rejected because NA03 already uses pigeons. Its replacement remains unselected; the proposal reserves a species-neutral flying filename. The draft also repeats log/stone/crate families and includes potential small-bird/silhouette overlaps, now explicitly marked for selection review. Carry focal point, life detail, local reference, campaign comparison and intended relative scale into each future focused stage packet. These requirements are confirmed direction, not approval of the draft lineup, new generation authorization or a new user approval loop. Accepted assets and active EU03 work are unchanged.

## 2026-09-19 — Remaining-stage direction and Workflow V3 approved

The user explicitly approved the proposal after reviewing bounded parallel execution and inclusion of the remaining-continent plan. Approval covers the twelve themes and full-stage scope, visual requirements, new-atlas pilot direction and implementation of one coordinator with at most two workers. The coordinator owns shared state/configuration, integration and publication. Workers receive only pinned stage/file rules and actual reference pixels, work in isolated locations and return compact manifests. Landscape continuity stays with one worker; the hazard worker owns both cells of the ground atlas and all flying frames. No nested managers, new image API runner, duplicate skill or fixed subscription-usage guarantee is introduced.

Workflow V3 adds focused future-stage/hazard readiness packets and per-file jobs with guarded atomic state updates, ownership/hash checks and immutable recovery evidence. Existing help/resume/revision commands and release/user-approval gates are preserved. Stale active instructions to continue from SA02 or regenerate the accepted batch are corrected; historical records are retained. All 27 NA/SA/EU landscapes remain unchanged and approved.

This approval does not supply an unknown replacement hazard, verify local wildlife references, accept generated artwork or waive dependent calibration/contract promotion. The initial duplicate/scale risks and new brief details remain explicit selection review items. Full-stage production remains blocked while runtime registration, hazard validation and the other recorded prerequisites are pending. The approved next implementation scope is to resolve those prerequisites and then measure AF01 as the first full-stage pilot before increasing concurrency.


## 2026-09-19 — Shared expansion support and three-stage Africa trial

The user clarified that the trial should cover AF01–AF03 and authorized completing shared preparation. AF01 is the first acceptance checkpoint within that trial; two workers may operate within one active stage, with no nested managers or simultaneous three-stage production. Evaluate the whole Africa trial before increasing concurrency. Setup is reusable across continents.

Added a shared 21-ID catalog with pending-stage isolation, complete measured release installation, source-anchor support in both hazard render paths, catalog-aware selectors/previews/save import, and structural atlas validators/integration helper. Original nine-stage artwork, geometry, physics and signatures are preserved. Tooling tests use synthetic fixtures and do not count as actual new-stage gameplay acceptance.

Africa selections replace duplicate log/stone/crate/pigeon directions with a crested porcupine, thin clay crust plates, forked camelthorn, tagine brazier, sandgrouse and white stork as recorded in the proposal. Regional references support those design selections; exact-site sightings and untested visual scale are not claimed. Actual reference pixel packs remain pending because direct image retrieval returned 403/HTML during setup. Existing dependent calibration/contract-promotion remains an evidence gate; no approval or test result is fabricated to bypass it. No image generation was performed in this change.

## 2026-09-19 — AF01 reference pack and dependent calibration review increment

The user authorized completing the outstanding preparation. AF01 now has six inspected, nonempty reference photographs with source credits, licenses, dimensions and SHA-256 hashes in `assets/references/africa/af01/REFERENCES.json`. Focused output mappings pair the relevant photos with accepted game-style references. An empty file cannot satisfy the resolver's reference gate. AF02/AF03 references remain just-in-time preparation before those stages.

Actual runtime tests found five HIGH flyers that could not be slid under by both characters and four wide ground hazards without an adequate timed jump window. A stricter timing-span check also required small reductions to the fallen log and Andean flamingo. Eleven hazard scales and five ground contact offsets were corrected around the established character reference; collider proportions, character settings, artwork, movement/altitude constants and Slide duration remain unchanged. Active canonical stages now derive character/hazard/finish gameplay anchors from fixed Y=410 rather than a visually adjusted GROUND seam. Packaged legacy surface fallback remains intact.

`docs/phase8-qa/DEPENDENT_CALIBRATION_QA.md` records measurements, visual limitations and the deployed review checklist. Timed tests exercise complete crossings using real runtime geometry, including one-frame collision ordering. Passing checks does not accept the smaller props or promote the landscape contract. Both remain subject to deployed user review; production stays disabled, and no Africa artwork was generated. The historical .70/.75 Slide prose discrepancy is not resolved by this work.

## 2026-09-19 — Africa production proceeds; calibration follow-up separately owned

The user reported a separate agent is implementing character/hazard auto-calibration, will pass it `DEPENDENT_CALIBRATION_QA.md`, and explicitly instructed this workflow to move forward. Adopt the accepted landscape contract for eligible remaining stages and enable AF01 production. Existing calibration review/adjustments remain open in that separate workstream; this decision does not record visual acceptance of the eleven hazard adjustments or declare all gameplay calibration finished. It supersedes the requirement to complete that review before remaining-stage generation.

AF01's content, six inspected reference photos and shared integration/validation tooling are ready. The next command is `In claudejones/candcgame: build stage AF01.` Continue the AF01–AF03 trial one active stage at a time with at most two workers. Keep per-stage reference/identity checks, actual new-hazard fit/collision checks, complete measured integration, CI/Pages and deployed user acceptance. Later reference packs remain agent work before their stages. Preserve concurrent auto-calibration/editor changes; no runtime settings or artwork are changed by this readiness update.

## 2026-09-19 — Retain calibration evidence; editor-next owns final comparison

The user relayed the calibration agent's handoff: retain `DEPENDENT_CALIBRATION_QA.md` and regression tests, pause independent legacy-prototype tuning, and use editor-next as the intended calibration interface. Reorganized the report into runtime correctness/tests, eleven scale/contact comparison candidates, and unresolved visual findings. The eleven deployed values remain unchanged and unaccepted; no setting is silently promoted, overwritten or reverted. The new tool alone does not satisfy artwork or contract-acceptance gates.

Preserved historical measurement data and added a read-only, hash-pinned 10ms audit reproducer. Its input-lead grid (0.05–1.60s) and crossing range (-1.9–1.9s) differ from the retained regression test's 20ms grid (0.06–1.20s) and crossing range (-1.5–1.5s); both simulate 60/120 Hz. Documented a contact worksheet's 2172/2048 base-scale mismatch as a review finding, without changing candidate offsets. Africa readiness and its per-stage gates remain as authorized.
...
## 2026-09-19 — AF02 measured source cleanup authorized

The user explicitly approved the documented deterministic AF02 cleanup after targeted imagegen edits retained alpha/anchor defects. Scope: remove alpha=1 residue from the two hazard atlases, shift OBJECT_ATLAS cells +7/−8 source pixels, and align GROUND by −6 source pixels with transparent upper and solid lower ownership bands plus copied bottom overscan. Preserve dimensions, substantive RGB, FAR/MID and all approved assets. This is a scoped source-art correction, not a change to general generation rules or a gameplay calibration approval. Evidence: `docs/phase8-qa/AF02_ASSET_QA.md`; asset-ready handoff: `config/asset-handoffs/af02.json`.

## 2026-09-20 — Bounded workflow execution and recovery

User approved implementing the audit of AF01/AF02 inefficiencies before the next continent trial. Keep the successful short command interface; reduce startup and worker payloads, preserve exact historical evidence outside active state, resolve designated recovery branches, diagnose repeated defects, and reuse unchanged local technical checks. Narrow content-preserving candidate corrections follow ASSET_TECHNICAL_FINISHING.md; accepted artwork remains protected. The connected publisher automates existing exact-SHA CI/same-tree/Pages gates with durable checkpoints; it does not remove them or grant artwork acceptance. Asset readiness remains separate from Workbench calibration and playable release. No fixed usage percentage, context capacity or measured parallel speedup is claimed. AF03's existing recovery work must be resumed, not overwritten or inferred complete from stale main state. Asia begins only after the current stage handoff and explicit advance.
