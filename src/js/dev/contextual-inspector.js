(() => {
  ...
  const field=(label,path,value,step=1)=>{
    ...
    down.addEventListener("click",()=>{input.stepDown();commit(input.value);});up.addEventListener("click",()=>{input.stepUp();commit(input.value);});input.addEventListener("change",()=>commit(input.value));
  ...
  function mount(){
    ...
    function render(){
      panel.hidden=window.CC_APP.mode!=="design";if(panel.hidden)return;const s=window.CC_DESIGN_SELECTION.current;const stage=window.CC_DESIGN_DRAFT.getStage(s.stageId);panel.replaceChildren();
      ...
      if(s.type==="landscape"){
        ...
        const collision=group("COLLISION");[["Width","w"],["Height","h"],["Offset X","x"],["Offset Y","y"]].forEach(([label,k])=>collision.append(field(label,`hazards.${s.assetIndex}.collision.${k}`,h.collision[k],0.01)));panel.append(source,crop,transform,collision,note("Atlas source region is separate from fine crop."));
      ...
      const actions=document.createElement("div");actions.className="inspector-actions";const status=document.createElement("span");status.className="dirty-status";status.textContent=window.CC_DESIGN_DRAFT.isDirty(s.stageId)?`Unsaved changes • ${window.CC_DESIGN_DRAFT.dirtyCount} stage(s)`:"Saved locally • no unsaved changes";
      ...
      const gameExport=document.createElement("button");gameExport.type="button";gameExport.className="export-game";gameExport.textContent="EXPORT GAME CONFIG";gameExport.onclick=()=>download(window.CC_DESIGN_DRAFT.exportGame(),"cc-world-game-config.json");
