(() => {
  ...
  function apply(){
    ...
    const doc=w.document,select=doc.getElementById("stateSelect");if(select){select.value=state;select.dispatchEvent(new Event("change",{bubbles:true}));}
    const label=doc.getElementById("characterLabel");if(label&&label.textContent.toLowerCase()!==character){doc.getElementById("toggleCharacter")?.click();}
  }
