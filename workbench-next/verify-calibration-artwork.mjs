...
const report={proposals:proposals.map(r=>({id:r.id,ready:r.ready,warnings:r.warnings,changes:r.changes,windows:r.reports.map(x=>({flight:x.flight,action:x.action,minimumMs:x.minimumMs,characters:x.characters}))})),sequences};
if(process.argv[3])fs.writeFileSync(process.argv[3],JSON.stringify(report,null,2)+'\n');
console.log(`${proposals.length} real atlases measured; ${proposals.filter(r=>r.ready).length} proposals meet Standard; ${proposals.filter(r=>!r.ready).length} correctly require adjustment. ${sequences.length} stage/profile sequences proved for both characters at 60 Hz. Approved image bytes were not changed.`);
