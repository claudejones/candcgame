...
export function calibrationDefaults(placement,stages,config){
 ...
 return {version:2,profile:'standard',profiles:{easy:profile(ground,fly,150,2.6,2.8,6,1),standard:profile(ground,fly,120,1.75,2.2,8,2),hard:profile(ground*1.2,fly*1.2,80,1.1,1.6,12,2)},stages:Object.fromEntries(stages.map(s=>{const id=s.stage||s,originY=410+['claude','constance'].reduce((sum,who)=>sum+(placement[`character:${who}`]?.footOffset||0)+(placement[`grounding:${id}:${who}`]?.groundOffset||0),0)/2;return [id,{originY,pathY:originY,characterFollow:{claude:true,constance:true}}];})),hazards:Object.fromEntries(Object.entries(placement).filter(([id])=>id.startsWith('hazard:')).map(([id])=>[id,{follow:true,enabled:true,locks:[],stamp:''}]))};
}
export function validateCalibration(c,baseline,placement,version=2){
 const exact=(a,b)=>a&&typeof a==='object'&&!Array.isArray(a)&&Object.keys(a).sort().join('|')===Object.keys(b).sort().join('|');
 if(!exact(c,baseline)||c.version!==version||!Object.hasOwn(baseline.profiles,c.profile)||!exact(c.profiles,baseline.profiles)||!exact(c.stages,baseline.stages)||!exact(c.hazards,baseline.hazards))throw new Error('Invalid calibration configuration.');
 ...
 for(const p of Object.values(c.stages)){
  if(!exact(p,version===1?{originY:0,pathY:0}:{originY:0,pathY:0,characterFollow:0})||![p.originY,p.pathY].every(n=>Number.isFinite(n)&&n>=-200&&n<=1200))throw new Error('Invalid stage pathway. Y must be between -200 and 1200.');
  if(version===2&&(!exact(p.characterFollow,{claude:0,constance:0})||Object.values(p.characterFollow).some(v=>typeof v!=='boolean')))throw new Error('Invalid character pathway links.');
 }
...
export function upgradeCalibration(c,baseline,placement){
 validateCalibration(c,baseline,placement,1);
 const upgraded=clone(c);upgraded.version=2;
 for(const p of Object.values(upgraded.stages))p.characterFollow={claude:true,constance:true};
 return validateCalibration(upgraded,baseline,placement);
}
...
export const characterPathShift=(draft,stage,who,baseline=false)=>(baseline?draft.calibrationBaseline:draft.calibration)?.stages[stage]?.characterFollow?.[who]===false?0:pathShift(draft,stage,baseline);
