...
export function calibrationStamp(draft,item,config){
 ...
 const input=JSON.stringify([CALIBRATION_VERSION,chars,characterFrames,characterCrops,draft.placement[item.id],draft.frames[item.id],draft.value[item.id],draft.provenance?.assets[item.asset],draft.calibration.stages[item.stage],draft.calibration.profile,draft.calibration.profiles[draft.calibration.profile],draft.calibration.hazards[item.id].follow,draft.calibration.hazards[item.id].locks,config.jump,config.actions,config.state,config.visibleMeta]);
 let h=2166136261;for(let i=0;i<input.length;i++)h=Math.imul(h^input.charCodeAt(i),16777619);return `cal1-${(h>>>0).toString(16)}`;
}
